...
pad_width = int(sys.argv[5])
pad_higth = int(sys.argv[6])
angle_tran = int(sys.argv[7])
# 绝对路径
cur_path = os.getcwd()
...
def xml_process(xml_file ,dst_xml_path, new_width, new_higth, angle):
    ...
    for i in range(len(root.getElementsByTagName("bndbox"))):
        ...
        item_xmin.firstChild.data = min_x
        item_ymin.firstChild.data = min_y
        item_xmax.firstChild.data = max_x
        item_ymax.firstChild.data = max_y
...
for image_file in os.listdir(src_img_path):
    ...
    dst_pad_img = pad_image(src_img, pad_width, pad_higth)
...
for xml_file in os.listdir(src_xml_path):
    ...
    xml_process(xml_name, dst_xml_path, pad_width, pad_higth, angle_tran)
