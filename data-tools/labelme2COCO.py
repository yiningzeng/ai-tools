...
path = "/media/baymin/项目/daily-data/a/"
save_json='/media/baymin/项目/daily-data/a/测试集集所有.json'
...
class labelme2coco(object):
    ...
    def image(self,data,num, json_file_name):
        sourceImgFileName = json_file_name.split(".")[0] + ".jpg"
        image={}
        # img = utils.img_b64_to_array(data['imageData'])  # 解析原图片数据
        # img=io.imread(data['imagePath']) # 通过图片路径打开图片
        img = cv2.imread(sourceImgFileName, 0)
        height, width = img.shape[:2]
        img = None
        image['height']=height
        image['width'] = width
        image['id']=num+1
        image['file_name'] = os.path.basename(sourceImgFileName)
...
if __name__ == '__main__':
    ...
    labelme2coco(labelme_json, save_json)